...
![](/imgs/mailgun/mailgun-3.png)

You'll now need to copy the `Private API Key` from _Settings_ > _API Keys_

![](/imgs/mailgun/mailgun-4.png)

Then paste this into your ZippyBox provider configuration.

![](/imgs/mailgun/mailgun-5.png)

## Tracking

To enable tracking, select _Sending_ then _Webhooks_ from the menu and click the _Add Webhook_ button.

![](/imgs/mailgun/mailgun-6.png)

The URL depends on your domain, but must end with `/api/v1/webhooks/mailgun`. For example, if ZippyBox is installed at `https://campaigns.marketing.com`, then each webhook should point to `https://campaigns.marketing.com/api/v1/webhooks/mailgun`.

![](/imgs/mailgun/mailgun-7.png)

You'll need to add an entry for each type of event you want to track, but the URL is the same for each of them.

![](/imgs/mailgun/mailgun-8.png)
