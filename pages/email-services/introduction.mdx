...
## Introduction
ZippyBox must be integrated with at least one email service provider in order for your emails to be dispatched. You can have multiple email services configured at one given time. For example, you may wish to send newsletters via SES and transactional emails via Postmark.
...
ZippyBox currently integrates with the following five email service providers:

- [AWS SES](/email-services/aws-ses)
