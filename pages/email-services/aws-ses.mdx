...
Selecting tags for the user is optional, so we're not going to enter any at this stage. Once the user has been created you should now be able to copy and paste the access key and secret access key to the ZippyBox provider configuration.

![](/imgs/aws/aws-15.png)

We're now ready to configure the provider with ZippyBox.
...
You'll need to configure Amazon's SNS (Simple Notification Service) in order for it to send the required webhooks to ZippyBox, so navigate to the SNS page from the AWS Management Console.
