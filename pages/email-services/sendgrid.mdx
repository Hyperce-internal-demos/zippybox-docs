...
![](/imgs/sendgrid/sendgrid-1.png)

Generate an API key and copy it to your ZippyBox configuration.

![](/imgs/sendgrid/sendgrid-2.png)

Check the `I've integrated the code above` box and click Next.

Send a test e-mail from ZippyBox and if it works successfully click _Verify integration_.

## Tracking

![](/imgs/sendgrid/sendgrid-3.png)
...
![](/imgs/sendgrid/sendgrid-5.png)

To enable tracking within ZippyBox you'll need to enable event notifications from _Settings_ > _Mail Settings_.

The `HTTP POST URL` depends on your domain, but must end with `/api/v1/webhooks/sendgrid`. For example, if ZippyBox is installed at `https://campaigns.marketing.com`, then each webhook should point to `https://campaigns.marketing.com/api/v1/webhooks/sendgrid`.
