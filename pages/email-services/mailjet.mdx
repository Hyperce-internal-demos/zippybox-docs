...
Finally, in order to integrate ZippyBox with Mailjet, you need to copy the `API Key` and the `API Secret Key`, which are [generated automatically](https://app.mailjet.com/account/api_keys) when your account is created, into your ZippyBox provider configuration. 

![](/imgs/mailjet/mailjet-1.png)
...
By default the tracking options for `opens` and `clicks` are activated for all new accounts. These settings can be changed on your [Account Preferences page](https://app.mailjet.com/account/settings), but keep in mind that ZippyBox will always override them with the values that you choose in the application when creating a new campaign.
...
![](/imgs/mailjet/mailjet-2.png)

The URL depends on your domain, but must end with `/api/v1/webhooks/mailjet`. For example, if ZippyBox is installed at `https://campaigns.marketing.com`, then each webhook should point to `https://campaigns.marketing.com/api/v1/webhooks/mailjet`.

You'll need to add an entry for each type of event you want to track, but the URL is the same for each one of them.

If you want to limit the number of calls, you can choose to group together events that occurred over the last second. ZippyBox is able to handle both single and grouped events.
